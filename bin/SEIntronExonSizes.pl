...
sub SE_exon_intron_sizes {
    print "\nCalculating exon and intron sizes for neg IncDiff and for pos IncDiff SE events...\n\n";
    my @upstream_exon_size_array_neg_IncDiff = ();
    my @upstream_intron_size_array_neg_IncDiff = ();
    my @skipped_exon_size_array_neg_IncDiff = ();
    my @downstream_intron_size_array_neg_IncDiff = ();
    my @downstream_exon_size_array_neg_IncDiff = ();
    my @upstream_exon_size_array_pos_IncDiff = ();
    my @upstream_intron_size_array_pos_IncDiff = ();
    my @skipped_exon_size_array_pos_IncDiff = ();
    my @downstream_intron_size_array_pos_IncDiff = ();
        ...
        if ($split_line[19] <= $FDR)  {
            ...
            if ($strand eq "+") {
                if ($amplitude < 0) {
                    ...
                    push(@upstream_intron_size_array_neg_IncDiff, $split_line[5]-$split_line[8]-1);
                    push(@skipped_exon_size_array_neg_IncDiff, $split_line[6]-$split_line[5]+1);
                    push(@downstream_intron_size_array_neg_IncDiff, $split_line[9]-$split_line[6]-1);
                    ...
                    push(@upstream_intron_size_array_pos_IncDiff, $split_line[5]-$split_line[8]-1);
                    push(@skipped_exon_size_array_pos_IncDiff, $split_line[6]-$split_line[5]+1);
                    push(@downstream_intron_size_array_pos_IncDiff, $split_line[9]-$split_line[6]-1);
                ...
                if ($amplitude < 0) {
                    ...
                    push(@upstream_intron_size_array_neg_IncDiff, $split_line[9]-$split_line[6]-1);
                    push(@skipped_exon_size_array_neg_IncDiff, $split_line[6]-$split_line[5]+1);
                    push(@downstream_intron_size_array_neg_IncDiff, $split_line[5]-$split_line[8]-1);
                    ...
                    push(@upstream_intron_size_array_pos_IncDiff, $split_line[9]-$split_line[6]-1);
                    push(@skipped_exon_size_array_pos_IncDiff, $split_line[6]-$split_line[5]+1);
                    push(@downstream_intron_size_array_pos_IncDiff, $split_line[5]-$split_line[8]-1);
    ...
    my $output_file_neg_IncDiff = $outdir."\/2_intron_exon_sizes_neg_IncDiff.tsv";
    open(OUTneg, ">$output_file_neg_IncDiff") or die "couldn't open output file";
    print OUTneg "Upstream exon\tUpstream intron\tSkipped exon\tDownstream intron\tDownstream exon\n";
    my $neg_IncDiff_array_length = @upstream_exon_size_array_neg_IncDiff;
    for (my $i = 0; $i < $neg_IncDiff_array_length; $i++) {
        print OUTneg $upstream_exon_size_array_neg_IncDiff[$i], "\t", $upstream_intron_size_array_neg_IncDiff[$i], "\t", $skipped_exon_size_array_neg_IncDiff[$i], "\t", $downstream_intron_size_array_neg_IncDiff[$i], "\t", $downstream_exon_size_array_neg_IncDiff[$i], "\n";
    }
    close(OUTneg);


    my $output_file_pos_IncDiff = $outdir."\/3_intron_exon_sizes_pos_IncDiff.tsv";
    open(OUTpos, ">$output_file_pos_IncDiff") or die "couldn't open output file";
    print OUTpos "Upstream exon\tUpstream intron\tSkipped exon\tDownstream intron\tDownstream exon\n";
    my $pos_IncDiff_array_length = @upstream_exon_size_array_pos_IncDiff;
    for (my $i = 0; $i < $pos_IncDiff_array_length; $i++) {
        print OUTpos $upstream_exon_size_array_pos_IncDiff[$i], "\t", $upstream_intron_size_array_pos_IncDiff[$i], "\t", $skipped_exon_size_array_pos_IncDiff[$i], "\t", $downstream_intron_size_array_pos_IncDiff[$i], "\t", $downstream_exon_size_array_pos_IncDiff[$i], "\n";
    }
    close(OUTpos);


    my $output_file = $outdir."\/1_intron_exon_size_SUMMARY.tsv";
    open(OUTstats, ">$output_file") or die "couldn't open output file";
    print OUTstats "Skipped exon file: ", $basename, "\tFDR = ", $FDR, "\n\tAve upstream exon\tAve upstream intron\tAve skipped exon\tAve downstream intron\tAve downstream exon\n";

    my @averages_pos_IncDiff = ();
    push(@averages_pos_IncDiff, average_of_array(@upstream_exon_size_array_pos_IncDiff));
    push(@averages_pos_IncDiff, average_of_array(@upstream_intron_size_array_pos_IncDiff));
    push(@averages_pos_IncDiff, average_of_array(@skipped_exon_size_array_pos_IncDiff));
    push(@averages_pos_IncDiff, average_of_array(@downstream_intron_size_array_pos_IncDiff));
    ...
    push(@averages_neg_IncDiff, average_of_array(@upstream_intron_size_array_neg_IncDiff));
    push(@averages_neg_IncDiff, average_of_array(@skipped_exon_size_array_neg_IncDiff));
    push(@averages_neg_IncDiff, average_of_array(@downstream_intron_size_array_neg_IncDiff));
...
sub annotated_intron_exon_sizes {
    print "Calculating annotated intron/exon sizes at potential exon skipping configurations...\n";
    my @upstream_exon_size_array = ();
    my @upstream_intron_size_array = ();
    my @skipped_exon_size_array = ();
    my @downstream_intron_size_array = ();
        ...
        if ($strand eq "+") {
            if ($block_number >2) {
                for (my $z = 2; $z < $block_number; $z++) {
                    push(@upstream_exon_size_array, $block_sizes[$z-2]);
                    push(@upstream_intron_size_array, $block_starts[$z-1] - ($block_starts[$z-2] + $block_sizes[$z-2]));
                    push(@skipped_exon_size_array, $block_sizes[$z-1]);
                    push(@downstream_intron_size_array, $block_starts[$z] - ($block_starts[$z-1] + $block_sizes[$z-1]));
            ...
            if ($block_number >2) {
                for (my $z = 2; $z < $block_number; $z++) {
                    push(@downstream_exon_size_array, $block_sizes[$z-2]);
                    push(@downstream_intron_size_array, $block_starts[$z-1] - ($block_starts[$z-2] + $block_sizes[$z-2]));
                    push(@skipped_exon_size_array, $block_sizes[$z-1]);
                    push(@upstream_intron_size_array, $block_starts[$z] - ($block_starts[$z-1] + $block_sizes[$z-1]));
    ...
    my $output_file_annotation = $outdir."\/4_intron_exon_sizes_annotated_exons.tsv";
    open(OUTannot, ">$output_file_annotation") or die "couldn't open output file";
    print OUTannot "Upstream exon\tUpstream intron\tSkipped exon\tDownstream intron\tDownstream exon\n";
    my $annotated_array_length = @upstream_exon_size_array;
    for (my $i = 0; $i < $annotated_array_length; $i++) {
        print OUTannot $upstream_exon_size_array[$i], "\t", $upstream_intron_size_array[$i], "\t", $skipped_exon_size_array[$i], "\t", $downstream_intron_size_array[$i], "\t", $downstream_exon_size_array[$i], "\n";
    }
    close(OUTannot);

    my @averages = ();
    push(@averages, average_of_array(@upstream_exon_size_array));
    push(@averages, average_of_array(@upstream_intron_size_array));
    push(@averages, average_of_array(@skipped_exon_size_array));
    push(@averages, average_of_array(@downstream_intron_size_array));
